...
class CSS
{
    ...
    public static function init()
    {
        if(!empty(Config::$options['assets']['remove_unused_css'])) {
            add_action('wp', array('Perfmatters\CSS', 'add_buffer_action'));
            add_action('wp_ajax_perfmatters_clear_post_used_css', array('Perfmatters\CSS', 'clear_post_used_css'));
        }
    }

    //add function to buffer
    public static function add_buffer_action() {
        if(!perfmatters_get_post_meta('perfmatters_exclude_unused_css') && !empty(apply_filters('perfmatters_remove_unused_css', true))) {
        ...
        if(!empty($stylesheets)) {
            ...
            foreach($stylesheets as $key => $stylesheet) {
                ...
                if(!$used_css_exists) {
                    ...
                    $url = explode('?', $stylesheet[1])[0];
                    $file = str_replace('/wp-content', '', WP_CONTENT_DIR) . wp_make_link_relative($url);
            ...
            if(!empty($used_css_string)) {
                file_put_contents($used_css_path, $used_css_string);
            }
