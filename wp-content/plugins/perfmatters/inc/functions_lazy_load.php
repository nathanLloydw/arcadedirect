	...
	if(!empty($pictures)) {
		...
		foreach($pictures as $picture) {
			...
			if(!perfmatters_lazyload_excluded($picture[1], perfmatters_lazyload_forced_atts())) {
				...
				if(perfmatters_lazyload_excluded($picture[0], perfmatters_lazyload_excluded_atts())) {
					continue;
				}
			}
...
function perfmatters_print_lazy_load_css() {
	...
	if(!empty($options['lazyload']['fade_in'])) {
		$styles.= '.perfmatters-lazy:not(picture),.perfmatters-lazy>img{opacity:0}.perfmatters-lazy.loaded,.perfmatters-lazy>img.loaded,.perfmatters-lazy[data-was-processed=true]{opacity:1;transition:opacity ' . apply_filters('perfmatters_fade_in_speed', 500) . 'ms}';
	}
