...
function perfmatters_assets_js_init() {
	...
	$delay_check = !empty(apply_filters('perfmatters_delay_js', !empty($perfmatters_options['assets']['delay_js']))) && !perfmatters_get_post_meta('perfmatters_exclude_delay_js');
	...
	if($defer_check || $delay_check) {
		...
		//actions + filters
		add_filter('perfmatters_output_buffer_template_redirect', 'perfmatters_optimize_js', 2);
...
function perfmatters_optimize_js($html) {
	...
	foreach($matches[0] as $i => $tag) {
		...
		//delay javascript
		if(!empty(apply_filters('perfmatters_delay_js', !empty($perfmatters_options['assets']['delay_js']))) && !perfmatters_get_post_meta('perfmatters_exclude_delay_js')) {

			$delay_flag = false;

			$delay_js_behavior = apply_filters('perfmatters_delay_js_behavior', $perfmatters_options['assets']['delay_js_behavior'] ?? '');
...
function perfmatters_print_delay_js() {
	...
	$perfmatters_options = get_option('perfmatters_options');
	$timeout = apply_filters('perfmatters_delay_js_timeout', !empty($perfmatters_options['assets']['delay_timeout']) ? 10 : '');

  	if(!empty(apply_filters('perfmatters_delay_js', !empty($perfmatters_options['assets']['delay_js'])))) {
  		
  		if(empty(apply_filters('perfmatters_delay_js_behavior', $perfmatters_options['assets']['delay_js_behavior'] ?? ''))) {
